...
trans_details = st.sidebar.multiselect('Select account details you want to view', TRANSACTION_COLUMNS)
...
# MemNumbers = lib.queryTable(conn, MEMBER_TABLE, 'MemNum', *).MemNum
...
if memberID:
    ...
    #st.dataframe(df.style.highlight_max(100))

    #pd.pivot_table(df)

    x = 10

    #selectable_data_table(df, key=None)